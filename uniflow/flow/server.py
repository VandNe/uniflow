...
from uniflow.flow.config import ExtractConfig, RaterConfig, TransformConfig, ExpandReduceConfig
...
class ExpandReduceServer:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        """
        # convert from dict to config for type checking
        self._config = ExpandReduceConfig(**config)

        self._flow_cls = FlowFactory.get(self._config.flow_name, flow_type=EXPANDREDUCE)
        self._num_thread = self._config.num_thread
        self._flow_queue = Queue(self._num_thread)
        for i in range(self._num_thread):
            with OpScope(name="thread_" + str(i)):
